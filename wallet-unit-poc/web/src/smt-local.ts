...
export async function loadSmtEngine(
  ...
  if (!wasmStarted) {
    await ensureWasmExecLoaded(onProgress, authorizing, signal);
    await startSmtWasm(onProgress, authorizing, signal);
    wasmStarted = true;
  }
...
// wasm_exec.js is classic UMD that assigns globalThis.Go; eval into Worker scope.
...
async function startSmtWasm(
  ...
  // smt.wasm is served as application/octet-stream which instantiateStreaming
  // rejects, so buffer first and use the non-streaming `instantiate`.
