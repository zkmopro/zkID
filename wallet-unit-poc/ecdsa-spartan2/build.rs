fn main() {
    #[cfg(feature = "native")]
    {
        ...
        if let Ok(witnesscalc_cache) = std::env::var("WITNESSCALC_PREBUILD_CACHE") {
            ...
            match target.as_str() {
                "aarch64-apple-ios-sim" | "aarch64-apple-ios" | "x86_64-apple-ios" => {
                    let cache_src = std::path::Path::new(&witnesscalc_cache);
                    let target_witnesscalc =
                        std::path::Path::new(&out_dir).join("witnesscalc");
        ...
        let staging_dir = std::path::Path::new(&out_dir).join("circuit_staging");
